...
// Get current position in story (NEW)
ipcMain.handle('progress:getPosition', async (event, { userId, storyId }) => {
    try {
        const progress = db.getProgress(userId, storyId);
        if (progress && progress.length > 0) {
            // Return the last completed segment
            const lastSegment = progress[progress.length - 1];
            return {
                success: true,
                segmentId: lastSegment.segment_id,
                completed: lastSegment.completed
            };
        }
        return {
            success: true,
            segmentId: 0, // Start from beginning
            completed: false
        };
    } catch (error) {
...
// Update current position (NEW - for tracking where user is without completing)
ipcMain.handle('progress:updatePosition', async (event, { userId, storyId, segmentId }) => {
    try {
        // Store current position in localStorage on the renderer side
        // This handler just acknowledges the update
        console.log(`Position updated: User ${userId}, Story ${storyId}, Segment ${segmentId}`);
        return { success: true };
    } catch (error) {
...
// Save completed segment (NEW)
ipcMain.handle('progress:saveSegment', async (event, { userId, storyId, segmentId }) => {
    try {
        db.saveProgress(userId, storyId, segmentId);
        
        // Check for progress badges
        checkAndAwardProgressBadges(userId);
